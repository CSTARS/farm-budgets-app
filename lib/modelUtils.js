...
function validate(obj, authorityModel, callback) {
  var i;

  for( i = 0; i < required.length; i++ ) {
    if( obj[required[i]] === undefined || obj[required[i]] === null ) {
      return callback(obj[i]+' is required');
    }
  }
...
function prepareQuery(query) {
  ...
  if( query.locality ) {
    ...
    } else if( query.locality.match(/^\[.*\]$/) ) {
      query.locality = JSON.parse(query.locality);
      query.locality.forEach(function(l){
        q.push({locality: clean(l)});
      });
    } else {
  ...
  if( query.authority ) {
    q.push({authority: query.authority});
  }
